...
bool IsMeSmaller(const std::pair<int,double>&p1, const std::pair<int,double>&p2)
{
  if (p1.second<p2.second)
  {
    return true;
...
double DecisionTree::CalcGiniIndex(std::vector<int> statistics)
{
...
double DecisionTree::CalcGiniIndex(std::list<Sample>&trainingSet)
{
  std::vector<int> counts = CountLabels(trainingSet);
  double gini = CalcGiniIndex(counts);
  return gini; 
}

double DecisionTree::CalcGiniIndex(std::list<Sample>&trainingSet, int featsId)
{
  ...
  //double Goriginal = CalcGiniIndex(trainingSet);
  double G1 = CalcGiniIndex(subSet1);
  double G2 = CalcGiniIndex(subSet2);
...
std::vector<std::pair<int,double> > DecisionTree::CalcGiniIndexBatch(std::list<Sample>&trainingSet)
{
  std::vector<std::pair<int,double> > results;
  int i = 0;
  for (std::map<int,int>::iterator it = allFeats.begin(); it != allFeats.end(); it++)
  {
    ...
    double gini = CalcGiniIndex(trainingSet,it->first);
    std::cerr <<i<<"\t"<<gini<<std::endl;
    i++;
    results.push_back(std::make_pair(it->first,gini));
  }
  return results;
}


std::pair<int,double> DecisionTree:: GetBestFeatIdGini(std::list<Sample>&trainingSet)
{
  std::vector<std::pair<int,double> > feats2Gini = CalcGiniIndexBatch(trainingSet);
  if (feats2Gini.size()==0)
  {
    return std::make_pair(-1,0);
  }
  std::sort(feats2Gini.begin(),feats2Gini.end(),IsMeSmaller);
 return feats2Gini[0]; 
}
...
void DecisionTree::ConstructDecisionTreeGini(std::list<Sample>&trainingSet,Node *root)
{
    double G0 = CalcGiniIndex(trainingSet);
    root->labelId = ComputeNodeLabelId(trainingSet);
    if (G0==0)
    {
        NodeCount++;
    ...
    std::pair<int,double> infopair=GetBestFeatIdGini(trainingSet);
    // no  attributes any more
    if (infopair.first==-1)
    {
      NodeCount++;
    ...
    if (subSet1.size()!=0)
    {
      Node *left = new Node;
      root->left =left;
      ConstructDecisionTreeGini(subSet1,left);
    }
    if (subSet2.size()!=0)
    {
      Node *right = new Node;
      root->right = right;
      ConstructDecisionTreeGini(subSet2,right);
    }
    NodeCount++;
    if (NodeCount%1==0)
    {
...
void DecisionTree::TrainGini(std::list<Sample> &trainingSet)
{
  root = new Node;
  ConstructDecisionTreeGini(trainingSet,root);
}
