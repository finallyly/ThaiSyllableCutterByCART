...
class DecisionTree
{
  public:
    ...
    std::pair<int,double> GetBestFeatIdGini(std::list<Sample>&trainingSet);
    std::string GetLabelNameById(int labelId);
    void Train(std::list<Sample>& trainingSet);
    void TrainGini(std::list<Sample>& trainingSet);
    void ConstructDecisionTree(std::list<Sample>&trainingSet,Node*root);
    void ConstructDecisionTreeGini(std::list<Sample>&trainingSet,Node*root);
    double CalcEntropy(std::list<Sample>&trainingSet);
    double CalcEntropy(std::vector<int> statistics);
    double CalcGiniIndex(std::vector<int> statistics);
    double CalcGiniIndex(std::list<Sample>&trainingSet);
    double CalcGiniIndex(std::list<Sample>&trainingSet,int featsId);
    std::vector<std::pair<int,double> > CalcGiniIndexBatch(std::list<Sample>&trainingSet);
